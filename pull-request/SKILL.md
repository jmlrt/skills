...
**Default: create as draft** unless user asks for ready-for-review.

**For non-current repo**: Add `--repo <owner/repo>`.

**For multiple PRs in one session**: Switch branches before each create: `git checkout <branch>` then repeat this phase.
...
**Issue linkage**: Use `Closes #X` or `Addresses #X` in PR body to link to issues.
